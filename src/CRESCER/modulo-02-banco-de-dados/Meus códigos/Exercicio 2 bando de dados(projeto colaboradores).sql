...
select Empregado, COUNT(1) as quantidadeDeProjetos
from Projeto
group by Empregado;

-- Projetos que geraram lucro
select distinct Projeto as ProjetosQueDeramLucro
from Projeto
where ValorFaturado > ValorRealizado

-- Projetos que geraram prejuizo
select distinct Projeto as ProjetosQueDeramPrejuizo, ((ValorFaturado*100)/ValorRealizado) - 100 as percentualPrejuizo
from Projeto
where ValorFaturado <= ValorRealizado;

-- Tabela com nome completo do empregado e só primeiro nome
select empregado as NomeCompleto,SUBSTRING(empregado,1,(CHARINDEX(' ',empregado+' '))) as PrimeiroNome
from projeto;
